    ...
    suspend fun getHoroscopesWith7DayCache(period: String = "TODAY", forceRefresh: Boolean = false): List<RashifalData> = withContext(ioDispatcher) {
            ...
            if (cachedEntities.size >= 12) {
                return@withContext cachedEntities.map { entity ->
                    RashifalData(
                        ...
                        luckyStoneHi = entity.luckyStoneHi.substringBefore(" (").trim(),
                        luckyStoneEn = com.example.astro.AstroNames.stoneEnFromHi(entity.luckyStoneHi),
                        luckyTimeHi = entity.luckyTimeHi,
                        luckyTimeEn = entity.luckyTimeEn,
        ...
        val entities = freshHoroscopes.map { item ->
            HoroscopeCacheEntity(
                ...
                luckyTimeHi = item.luckyTimeHi,
                luckyTimeEn = item.luckyTimeEn,
